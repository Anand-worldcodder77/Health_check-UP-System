...
const getStats = () => {
  ...
  const earnings = savedBookings.reduce((acc, curr) => {
    const price = parseInt(String(curr.amount || '').replace(/[^0-9]/g, ''), 10) || 0;
    return acc + price;
  }, 0);
  ...
  return {
    ...
    pendingBookings: savedBookings.filter((booking) => booking.status === 'pending').length,
    confirmedBookings: savedBookings.filter((booking) => booking.status === 'confirmed').length,
  };
};
...
const LabStats = () => {
  ...
  const cards = [
    { title: 'Total Patients', value: stats.totalPatients, icon: <Users size={24} />, color: 'bg-blue-500', trend: '+12% this month' },
    { title: 'Revenue (INR)', value: `Rs. ${stats.totalEarnings.toLocaleString()}`, icon: <IndianRupee size={24} />, color: 'bg-emerald-500', trend: 'Live Earnings' },
    { title: 'Active Pending', value: stats.pendingBookings, icon: <Activity size={24} />, color: 'bg-amber-500', trend: 'Action Required' },
    { title: 'Confirmed', value: stats.confirmedBookings, icon: <TrendingUp size={24} />, color: 'bg-[#009494]', trend: 'Ready for Report' },
  ];
  ...
  return (
    <div className="p-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
        {cards.map((card) => (
          <div key={card.title} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden group hover:-translate-y-2 transition-all duration-300">
      ...
      <div className="bg-slate-900 rounded-[50px] p-12 text-white flex flex-col md:flex-row items-center justify-between relative overflow-hidden shadow-2xl">
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-sm max-w-md font-medium leading-relaxed">
            Review pending bookings and keep collection follow-ups on time.
          </p>
